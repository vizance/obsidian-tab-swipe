...
導覽列下緣常駐一排小圓點，一個分頁一顆，目前所在的那顆會亮起來。它不會消失，所以你隨時都能一眼看出自己在第幾個，而不是只有剛滑完那一瞬間看得到。這件事在兩端循環的情況下特別重要。

圓點跟的是 Obsidian 的狀態，不是這個外掛的動作。你用內建分頁清單切換、點連結開新分頁、關掉一個分頁，圓點都會跟上。分頁少於兩個時整排會自己隱藏，因為只有一顆點等於沒講話。
